...
```bash
stock-trader$ sam build --use-container
```
...
```bash
stock-trader$ sam logs -n StockCheckerFunction --stack-name stock-trader --tail
```
...
```bash
stock-trader$ pip install pytest pytest-mock --user
stock-trader$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name stock-trader
```
